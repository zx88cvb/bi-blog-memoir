...
- Next.js App Router lives in `src/app` with route segments for the landing page (`page.tsx`), post detail (`posts/[slug]`), `about`, and `friendly-links`; shared layout is in `layout.tsx` with global styles in `globals.css`.
...
## Testing Guidelines
- Automated tests are not yet present; prioritize linting and manual verification of key routes (`/`, `/posts/[slug]`, `/about`, `/friendly-links`) across mobile and desktop.
